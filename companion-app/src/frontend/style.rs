...
use egui::{Color32, CornerRadius, FontFamily, Margin, Shadow, Stroke, ThemePreference};
...
pub fn main_font() -> FontFamily {
    FontFamily::Name("CustomFont".into())
}
pub fn strong_font() -> FontFamily {
    FontFamily::Name("CustomFont_Strong".into())
}
pub fn condensed_font() -> FontFamily {
    FontFamily::Name("Condensed_Numbers".into())
}
...
pub const FRAME_CORNER_RADIUS: CornerRadius = CornerRadius::same(20 + FRAME_PADDING.unsigned_abs());
pub const CUSTOM_FRAME: egui::containers::Frame = egui::containers::Frame {
    // inner and outer padding:
    inner_margin: Margin::symmetric(FRAME_PADDING + 4, FRAME_PADDING),
...
pub fn setup_global_application_style(cc: &CreationContext<'_>) {
    ...
    if let Some(winit_window) = &cc.winit_window {
        winit_window.set_theme(Some(winit::window::Theme::Dark));
    }
    ...
    style.text_styles.insert(
        ...
        egui::FontId::new(18.0, strong_font()),
    );
    style.text_styles.insert(
...
fn load_custom_fonts(cc: &CreationContext<'_>) {
    let mut fonts = egui::FontDefinitions::default();

    // Embed .ttf in binary and register it under CustomFont
    fonts.font_data.insert(
        "CustomFont".into(),
        egui::FontData::from_static(include_bytes!(
            "../../assets/fonts/IosevkaEtoile-Regular.ttf"
        ))
        .into(),
    );
    fonts.font_data.insert(
        "CustomFont_Strong".into(),
        egui::FontData::from_static(include_bytes!("../../assets/fonts/IosevkaEtoile-Heavy.ttf"))
            .into(),
    );
    fonts.font_data.insert(
        "Condensed_Numbers".into(),
        egui::FontData::from_static(include_bytes!("../../assets/fonts/Oswald-Regular.ttf")).into(),
    );

    // add lookup to font data to the registered families
    fonts
        .families
        .entry(main_font())
        .or_default()
    ...
    fonts
        ...
        .entry(strong_font())
        .or_default()
        .insert(0, "CustomFont_Strong".into());
    fonts
        .families
        .entry(condensed_font())
        .or_default()
        .insert(0, "Condensed_Numbers".into());

    // also override default egui font
    fonts
        .families
