...
impl SettingsSubsystem {
    ...
    pub fn draw_dev_options(&mut self, _ui: &mut egui::Ui) {
        #[cfg(debug_assertions)]
        {
            use egui::{Checkbox, Color32, Frame, RichText, Slider, Stroke, Widget};
            Frame::new()
                .inner_margin(10.0)
                .outer_margin(8.0)
                ...
                .show(_ui, |_ui| {
                    ...
                    Checkbox::new(&mut self._dev_on_hover_diagnostics, "On-hover diagnostics")
                        .ui(_ui);
                    _ui.ctx().style_mut(|s| {
                    ...
                    // Show widget hits
                    Checkbox::new(&mut self._dev_show_widget_hits, "Show widget hit by cursor")
                        .ui(_ui);
                    _ui.ctx().style_mut(|s| {
                    ...
                    for string_id in &self._dev_sliders {
                        ...
                        let mut value: f32 = _ui.ctx().memory(|mem| -> f32 {
                            mem.data.get_temp(cur_slider_id).unwrap_or(1.0)
                        });
